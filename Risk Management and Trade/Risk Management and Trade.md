...
两个公式之间的关系是:
1) 简单加权平均模型是对最近 m 个观测值的加权平均, 而无条件方差估计量对整个样本求平均。当 m=T_{total}时, 两者是等价的。

2) 简单加权模型中, 权重 $\frac{1}{m}$ 对所有观测是一样的, 而无条件方差估计量中, 所有观测的权重是 $\frac{1}{T_{total}}$, 相当于给予较远期的观测值较低权重。

3) 简单加权模型隐含地假设 $\mu_{R_{t+1}} = 0$, 即假设回报的均值为 0。而无条件方差估计量中, 先对回报序列进行去均值化。

所以从无条件方差估计量推导出简单加权模型, 需要作以下假设:
1) 令 $m = T_{total}$, 即考虑全部观测  
2) 令 $\mu_{R_{t+1}} = 0$

在此条件下, 无条件方差估计量可以简化为:

$$\hat{\sigma}^2_{R_{t+1}} = \frac{1}{T_{total}}\sum_{i=1}^{T_{total}}R_i^2 = \frac{1}{m}\sum_{j=0}^{m-1}R_{t-j}^2 = \sigma^2_{t+1}$$

因此, 简单加权模型可以视为无条件方差估计量在均值为 0、并对所有观测赋予等权重的特殊情况。当然, 简单加权模型并不能很好地刻画波动率的时变性和持续性, 因此需要更复杂的模型来捕捉这些特征。
