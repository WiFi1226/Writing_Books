...
$$
因为$Cov(\mathbf{Z}',\mathbf{u})= 0$,当 $N\rightarrow\infty$ 时，由大数定律 $\frac{1}{N}\sum_{i=1}^{N}z_i u_i$ 的平均值 (即 $\frac{1}{N}(\mathbf{Z}'\hat{\mathbf{u}})$) 将收敛到其期望值 $E(\mathbf{Z}'\hat{\mathbf{u}})=0$。因此可以从下式中定义出 IV 估计量 ：
$$
