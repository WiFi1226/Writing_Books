...
###### 数学原理
对 $\space \mathbf{Y} = \mathbf{X}\beta + \mathbf{u}$ 定义一个维度为 $k$ 的矩阵 $\mathbf{Z}(z_1, \ldots, z_k)$ 作为 $\mathbf{X}$ 的工具变量，矩阵 $\mathbf{Z}$ 满足：
$$
\begin{aligned}
...
\end{aligned}
$$
注1：如果 $E(u∣X)=0$ (即不存在设定偏误 $\rightarrow Cov(X_i,u_i)= 0$)，IV 估计量退化为 OLS 估计量。



与 $\mathbf{X}(x_1, \ldots, x_k)$ 有着相同的
